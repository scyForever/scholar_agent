...
- `ResearchPlanningSkill`：把研究主题拆成阶段任务，并把综述类写作约束继续分发到目标、任务、校验和风险
...
- 近期原文层：最近若干条 `role/content` 原始消息
- 重点提炼层：从偏好、主题、方法、数据、结论等句子中提取要点
- LLM 摘要层：通过现有 `LLMManager` 压缩近期窗口之外的历史诉求、约束、关注点、已确认结论和待跟进点

进入模型前，`MemoryContextBuilder` 会按预算组合短期三层记忆和长期召回结果，避免把完整历史无差别塞进 prompt。
...
- 外部搜索路径
- 工具调用信息
- 约束驱动的 `constraint_budget`

后续 `WriteAgent` 会从 `SearchResult.trace.local_rag` 中抽取本地片段和网页补充片段，直接写进生成提示词。
如果当前意图是综述生成，它还会优先使用清洗后的 `slots.topic` 作为主题，并把输出语言、组织方式、指定章节、最低参考文献数和引用格式一起写入 `survey_writer` 模板材料。
...
1. `arXiv` 不再直接吃自然语言原串  
   普通查询会被转成字段化检索式，优先匹配标题、摘要和关键词全覆盖，避免 arXiv 把空格 query 扩成过宽的 `OR` 检索。

2. 外部学术源的聚合不再由 citation 主导  
   当前跨源融合会综合：
   - 文本相关性
   - 时间新鲜度
   - 引用数的对数压缩分
...
同时在最终排序前做一次来源多样性重排，减少 `OpenAlex` 这类高 citation 数据源对 `arXiv` 的长期压制。
...
- `ResearchPlanningSkill`
  ...
  对综述类请求，还会把 `min_references / language / organization_style / required_sections / citation_style / outline_depth` 这些结构化写作约束同步注入计划目标、任务交付、里程碑、校验项和风险提示。
...
- `AnalyzeAgent`
  ...
  在真正调用 LLM 前，它会先按证据优先级重排候选论文，优先选择融合分高、摘要更完整、可获取全文、开放获取、跨源信息更完整、且更适合当前写作约束的论文。
- `DebateAgent`
...
长期记忆写入时会自动生成 `owner_key` 和 `keywords`，召回时按 `user_id` 做用户专属隔离，避免不同用户的偏好、论文和对话摘要互相污染。

当前版本面向新库 schema，不保留旧版本数据库迁移兼容逻辑。详细设计见 `docs/记忆管理机制.md`。
...
- Agent 评测当前包含 `120` 条 case，关注“任务有没有做成、流程是否按预期执行、短期/长期记忆行为是否符合预期、功能是否匹配、重复运行是否稳定”
- retrieval case 采用 `48` 条 exact title、`36` 条 semantic pair、`36` 条 semantic triple 的混合结构，用来把 `precision_at_k` 拉成多档分布
- agent case 额外包含 `20` 条记忆专项样本，分别验证短期原文层/重点提炼层/LLM 摘要层、当前用户长期记忆召回和其他用户长期记忆隔离
